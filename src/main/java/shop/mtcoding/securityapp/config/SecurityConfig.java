...
public class SecurityConfig {
    ...
    SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        // 6. httpBasic 정책해제 (BasicAuthenticationFilter 해제)
        http.httpBasic().disable();

        // 7. XSS (lucy 필터) - 다 막아준다
        ...
        http.exceptionHandling().authenticationEntryPoint((request, response, authException) -> {
            ...
            response.setContentType("text/plain; chatest=utf-8");
        ...
        http.exceptionHandling().accessDeniedHandler((request, response, accessDeniedException) -> {
            ...
            log.info("디버그 : 인증 실패 : " + accessDeniedException.getMessage());
            log.warn("디버그 : 인증 실패 : " + accessDeniedException.getMessage());
            log.error("디버그 : 인증 실패 : " + accessDeniedException.getMessage());
